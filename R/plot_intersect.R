...
#' hmpump=curva_bomba <- hmpump(Q_BC21_15, Hm_BC21_15)
#' hmsis=curva_sis <- hmsis(
...
plot_intersect <- function(hmsis, hmpump) {
  ...
  intersect=intersect_pump(hmsis, hmpump)*3600
    ...
    ggplot()+
    geom_line(data=hmpump[[1]],aes(x=q_bomba*3600, y=hm_bomba), color="red", size=1)+
    geom_line(data=hmsis[[1]],aes(x=q*3600, y=hm), color="blue", size=1)+
    labs(
      x = "Flow rate (m3/h)",
